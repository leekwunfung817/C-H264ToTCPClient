...
int CCPosVideoDemoDlg::CameraLogin(CString strIP, int nPort, CString strUserName, CString strPassword, int bCloud)
{
	...
	H264_DVR_SetupAlarmChan(lLogin);//ע���¼id��������Ϣ, ��������ʱ������Ӧ�ĵ�¼�豸��
...
int CCPosVideoDemoDlg::ConnectRealPlay(int nIndex, bool bOsd, int nChannel, CString strIP, int nPort, CString strUserName, CString strPassword, int bCloud, HWND hWnd)
{
	...
	if (m_LoginID <= 0)
	{
		login = CameraLogin(strIP, nPort, strUserName, strPassword, bCloud); //��¼���ѵõ�m_LoginID
		if (login <= 0)
		{
			return 0;
		}
	...
	if (m_nDecodeHandle == -1)
	{
		...
		if (H264_PLAY_OpenStream(m_videoIndex, &byFileHeadBuf, 1, SOURCE_BUF_MIN * 50))
		{
			...
			//H264_PLAY_SetDecCallBack(m_nIndex, nProc);//���ý���ص�, ���ú��ڻص����Լ�����
...
int CCPosVideoDemoDlg::CaptureBmpToRam(char *pBuf, int len)
{
	...
	if (m_LoginID > 0 && m_iPlayhandle > 0)
	{
		...
		LONG height = 0;
		char c = 0;
		lRetLen = H264_PLAY_CatchPicBuf(m_nDecodeHandle, &c, 0, &width, &height, 0);//ץȡBMPͼ���ڴ�
		int dib_length = width * height * 3 + 54;//BMPͼ��Ĵ洢�ռ�

		//H264_PLAY_CatchPic(m_nPlaydecHandle, filename, 0))//ֱ��ץȡ���ļ� ntype:0-bmp, 1-jpg
		lRetLen = H264_PLAY_CatchPicBuf(m_nDecodeHandle, pBuf, dib_length, &width, &height, 0);//ץȡBMPͼ���ڴ�
		if (lRetLen > 0)//���ش洢��С
		{
